...
{{"Job_Description_Match":"%","Missing_Keywords":"","Candidate_Summary":"","Experience":""}}
"""
...
if submit_button:
    if uploaded_file is not None:
        ...
        print(type(ast.literal_eval(response_text)))
        ...
        final_data =  ast.literal_eval(response_text)
        st.write(f"Job Description Match : --> {final_data['Job_Description_Match']}")
        st.write(f"Missing Keywords: --> {final_data['Missing_Keywords']}")
        st.write(f"Candidate Summary: --> {final_data['Candidate_Summary']}")
        st.write(f"Experience: --> {final_data['Experience']}")
        if int(final_data['Job_Description_Match'].replace("%","")) >= 80:
            st.write("Final decision: --> Move forward with hiring")
        else:
            st.write("Final decision: --> Not a Match")
